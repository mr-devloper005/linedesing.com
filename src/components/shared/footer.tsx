...
export function Footer() {
  ...
  if (recipe.footer === 'dense-footer') {
    return (
      <footer className="border-t border-[rgba(251,195,193,0.65)] bg-[linear-gradient(180deg,#fff8fb_0%,#fff5ef_100%)] text-[#3b2032]">
        <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
          <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr_1fr]">
            <div className="rounded-[2rem] border border-[rgba(251,195,193,0.65)] bg-white p-7 shadow-[0_18px_52px_rgba(176,100,132,0.12)]">
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-[rgba(251,195,193,0.65)] bg-[rgba(255,245,248,0.8)] p-1.5">
                  <img src="/favicon.png?v=20260401" alt={`${SITE_CONFIG.name} logo`} width="48" height="48" className="h-full w-full object-contain" />
                </div>
                <div>
                  <p className="text-lg font-semibold">{SITE_CONFIG.name}</p>
                  <p className="text-xs uppercase tracking-[0.24em] text-[#8f5e74]">{siteContent.footer.tagline}</p>
                </div>
              </div>
              <p className="mt-5 max-w-md text-sm leading-7 text-[#795569]">{SITE_CONFIG.description}</p>
              <div className="mt-5 flex flex-wrap gap-2">
                {spotlightTasks.map((task) => (
                  <Link key={task?.key} href={task?.route || '/'} className="inline-flex items-center gap-2 rounded-full border border-[rgba(251,195,193,0.7)] bg-[rgba(255,245,248,0.95)] px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#5d3249] hover:bg-white">
                    {task?.label}
                  </Link>
                ))}
              </div>
              {primaryTask ? (
                <Link href={primaryTask.route} className="mt-6 inline-flex items-center gap-2 rounded-full bg-[#FE81D4] px-4 py-2.5 text-sm font-semibold text-[#3b2032] hover:bg-[#f66bca]">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:col-span-2 lg:grid-cols-3">
              <div>
                <h3 className="text-xs font-semibold uppercase tracking-[0.24em] text-[#8f5e74]">All tasks</h3>
                <ul className="mt-4 space-y-3 text-sm text-[#5f3d50]">
                  {supportingTasks.map((task) => {
                    const Icon = taskIcons[task.key] || LayoutGrid
                    return (
                      <li key={task.key}><Link href={task.route} className="flex items-center gap-2 hover:text-[#3b2032]">{Icon ? <Icon className="h-4 w-4" /> : null}{task.label}</Link></li>
                    )
                  })}
                  {!supportingTasks.length ? footerLinks.platform.map((item: any) => (
                    <li key={item.name}><Link href={item.href} className="flex items-center gap-2 hover:text-[#3b2032]">{item.icon ? <item.icon className="h-4 w-4" /> : null}{item.name}</Link></li>
                  )) : null}
                </ul>
              </div>
              <div>
                <h3 className="text-xs font-semibold uppercase tracking-[0.24em] text-[#8f5e74]">Resources</h3>
                <ul className="mt-4 space-y-3 text-sm text-[#5f3d50]">
          ...
          <div className="mt-10 border-t border-[rgba(251,195,193,0.65)] pt-5 text-sm text-[#8f5e74]">&copy; {new Date().getFullYear()} {SITE_CONFIG.name}. All rights reserved.</div>
        </div>
      </footer>
    )
  }
